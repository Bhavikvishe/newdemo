...
export async function fetchPredictions(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        err?.error ??
        `detect failed with status ${res.status}`,
      );
    }
...
/**
 * Converts an image file to an optimized,
 * self-contained Data URL.
 *
 * Large images are scaled down so they can
 * safely be stored and displayed.
 */
...
// Restore stored images from IndexedDB
if (
  typeof window !==
  'undefined' &&
  window.indexedDB
