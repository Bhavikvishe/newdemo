...
export function MyDepartmentPage() {
  ...
  const view = useMemo(() => alerts.filter((a) => a.detection.department === me && (status === 'open' ? OPEN.includes(a.status) : status === 'resolved' ? a.status === 'resolved' : true)), [alerts, me, status]);

  const myOpen = alerts.filter((a) => a.detection.department === me && OPEN.includes(a.status));
  const myResolved = alerts.filter((a) => a.detection.department === me && a.status === 'resolved');
    ...
    <div>
      ...
      <div className="grid cols-12" style={{ gap: 16 }}>
        ...
        <div className="span-3"><Card className="h-full solid" style={{ padding: 14 }}>
          ...
          <b className="stat-num" style={{ color: 'var(--teal)' }}>{myResolved.length}</b>
        </Card></div>
        <div className="span-3"><Card className="h-full solid" style={{ padding: 14 }}>
