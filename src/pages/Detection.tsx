...
export function DetectionPage() {
  ...
  const selectPrediction = (idx: number) => {
    ...
    setResult((prev) => {
      ...
      return {
        ...
        riskScore:
          meta?.riskBase === 'critical'
            ? 95
            : meta?.riskBase === 'high'
              ? 80
              : 55,
        department: meta ? meta.primary : 'marine-operations',
        recommendedEquipment: meta
          ? meta.equipment
          : ['Survey ROV'],
        removalMethod: meta
          ? meta.removal
          : 'Standard marine salvage',
  ...
  const run = async (
    ...
    try {
      ...
      // -------------------------------------------------------------
      // IMPORTANT:
      // GPS comes from the backend/image metadata.
      // It is NOT calculated from the YOLO bounding box.
      // -------------------------------------------------------------
      const modelGps = resp.gps
        ? {
      ...
      const mappedClass: DetectionClass =
        mapClass(p.label) ?? 'shipwreck';
      ...
      const realDet: Detection = {
        ...
        // EXACT prediction confidence from YOLO.
        confidence: p.confidence,
        ...
        // REAL IMAGE GPS
        //
        // This must come from the backend response, which should read
        // EXIF GPS metadata from the uploaded image.
        //
        // There is NO bbox-based GPS calculation here anymore.
        // -----------------------------------------------------------
        gps: modelGps,
        ...
        notes:
          `Real model detection: ${p.label} (conf: ${(p.confidence * 100).toFixed(1)}%) via best.pt`,
        ...
        inferenceDetails: {
          ...
          imgsz:
            resp.debug?.imgsz ?? 640,

          iou:
            resp.debug?.iou ?? 0.7,

          modelPath:
            resp.debug?.model_path ??
            'E:\\newdemo\\best.pt',
  ...
  const meta =
    result
      ? CLASS_META[result.className]
      : null;

  const reportSource:
    DetectionReportSource | null =
    done && result
      ? {
        id: result.id,
  ...
  const noObjectReportSource:
    DetectionReportSource | null =
    noObjects && currentFile
      ? {
        id: currentFile.name,
  ...
  return (
    <div>
      <PageHead
        ...
        sub={
          demoMode
            ? 'Demo Mode (Simulated Mock Detections)'
            : 'Real AI Model Inference (best.pt) · 6 Classes'
        }
        right={
          <div
            className="row"
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <span>
                Detections in this mode
                are randomly generated
                synthetic simulations for
                offline UI testing and do{' '}
                <em>
                  not
                </em>{' '}
                reflect inferences from
                the trained{' '}
                <code>
                  best.pt
                </code>{' '}
          ...
          {!active ? (
            <>
              ...
              >
                ...
                >
                  {demoMode
                    ? 'Demo Mode Active — Simulated Pipeline'
                    ...
                    >
                      <span className="badge b-accent">
                        {allDetections.length >
                          ...
                          ).toUpperCase()} · ${pct(result.confidence)}`}
                      </span>
          ...
          {error ? (
            <Card className="h-full">
              ...
              >
                ...
                >
                  The backend inference
                  server encountered an
                  issue or is unreachable.
                  Ensure the Python API
                  is running:
                </p>
            ...
            <Card className="h-full">
              ...
              >
                ...
                >
                  ...
                  . Real model mode never
                  fabricates fake detections.
                </p>

                {noObjectReportSource && (
            ...
            <div
              ...
              style={{ gap: 16 }}
            >
              {/* SECTION 1: MODEL OUTPUT */}
              <Card>
                <CardHead
                  kt={
                    result.isRealModel
                      ? 'MODEL OUTPUT (RAW INFERENCE)'
                      : 'SIMULATED DEMO (RANDOM MOCK DATA)'
                  }
                  title={
                    allDetections.length >
                      1
                  ...
                  right={
                    ...
                    >
                      {result.isRealModel
                        ? 'Real AI Prediction (best.pt)'
                        : 'Simulated Demo (Not Real AI)'}
                    </Tag>
                  }
                />
                        ...
                        {allDetections.map(
                          (
                            ...
                            <button
                              key={idx}
                              type="button"
                              className={`btn btn-sm ${selectedPredIdx ===
                                  idx
                                  ? 'btn-primary'
                                  : 'btn-secondary'
                                }`}
              ...
              {/* SECTION 3: DEBUG TELEMETRY */}
              {debugInfo && (
                <Card>
                  <CardHead
                    kt="DEBUG TELEMETRY"
                    title="YOLO Diagnostics"
                  />
